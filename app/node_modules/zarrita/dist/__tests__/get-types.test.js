...
describe("Chunk type per dtype", () => {
    test("int8", async () => {
        let chunk = await zarr.get(mockArray());
    ...
    test("int64", async () => {
        ...
        expectType(chunk.data).toMatchInlineSnapshot(`BigInt64Array<ArrayBufferLike>`);
    });
    test("uint8", async () => {
        let chunk = await zarr.get(mockArray());
    ...
    test("uint16", async () => {
        ...
        expectType(chunk.data).toMatchInlineSnapshot(`Uint16Array<ArrayBufferLike>`);
    });
    test("uint32", async () => {
        let chunk = await zarr.get(mockArray());
        expectType(chunk).toMatchInlineSnapshot(`zarr.Chunk<"uint32">`);
        expectType(chunk.data).toMatchInlineSnapshot(`Uint32Array<ArrayBufferLike>`);
    });
    test("uint64", async () => {
        let chunk = await zarr.get(mockArray());
        expectType(chunk).toMatchInlineSnapshot(`zarr.Chunk<"uint64">`);
        expectType(chunk.data).toMatchInlineSnapshot(`BigUint64Array<ArrayBufferLike>`);
    });
    test("float32", async () => {
        let chunk = await zarr.get(mockArray());
        expectType(chunk).toMatchInlineSnapshot(`zarr.Chunk<"float32">`);
        expectType(chunk.data).toMatchInlineSnapshot(`Float32Array<ArrayBufferLike>`);
    });
    test("float64", async () => {
        let chunk = await zarr.get(mockArray());
        expectType(chunk).toMatchInlineSnapshot(`zarr.Chunk<"float64">`);
        expectType(chunk.data).toMatchInlineSnapshot(`Float64Array<ArrayBufferLike>`);
    });
    test("bool", async () => {
        let chunk = await zarr.get(mockArray());
        expectType(chunk).toMatchInlineSnapshot(`zarr.Chunk<"bool">`);
        expectType(chunk.data).toMatchInlineSnapshot(`zarr.BoolArray<ArrayBufferLike>`);
    });
    test("string", async () => {
        let chunk = await zarr.get(mockArray());
...
describe("Scalar type per union dtype (point selection)", () => {
    test("NumberDataType", async () => {
        let scalar = await zarr.get(mockArray(), [0, 0]);
...
describe("Selection-based return type", () => {
    ...
    test("point selection returns Scalar", async () => {
        let result = await zarr.get(mockArray(), [0, 0]);
